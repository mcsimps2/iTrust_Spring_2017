...
<div class="iTrustMenuContents">
	...
	<%
		...
		if(failureCount != 0) {
			...
			if (failureCount >= 3) //IP Ban message should appear if failure counts too great
			{
				msg = "Your IP has been banned for 15 minutes";
			}
	...
	<%
		//if(!loginFailureAction.needsCaptcha()){ //COMMENTED OUT: We no longer require the use of Captcha.  Instead, we are using IP Bans
	...
	<%
		//} else { //COMMENTED OUT: No longer using Captcha, only IP bans
	%>
	<!-- COMMENTED OUT ELSE STATEMENT
		<form method="post" action="/iTrust/login.jsp">
		MID<br />
